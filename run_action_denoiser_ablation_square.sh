...
# (conditioned on state) vs joint-denoiser — Square task, anchor A0.
#
...
#
# Run from repo root:
#   bash run_action_denoiser_ablation_square.sh
#
# Optional overrides via env:
#   EPOCHS=2 N_ROLLOUTS=2 bash run_action_denoiser_ablation_square.sh   # smoke test
...
JOINT_CKPT="$REPO_ROOT/diffusion_models/joint_unet_square_a0.pt"
ACTION_CKPT_COND="$REPO_ROOT/diffusion_models/action_a0_square.pt"         # state-conditioned (A0)
ACTION_CKPT_NOCOND="$REPO_ROOT/diffusion_models/action_a0_nocond_square.pt" # no-condition (A0)
