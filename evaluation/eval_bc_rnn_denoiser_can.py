#!/usr/bin/env python3
"""
Evaluate BC RNN policy on the Can task with optional action and joint denoisers.
...
Gaussian noise can be injected into observations (alpha_s) and actions (alpha_a)
to simulate deployment perturbations.

Usage:
    python evaluation/eval_bc_rnn_denoiser_can.py \\
        --bc_rnn_checkpoint checkpoints/bc_rnn_can/.../model_epoch_600.pth \\
        --action_denoiser   diffusion_models/action_a0_can.pt \\
        --joint_denoiser    diffusion_models/joint_a0_can.pt \\
        --alpha_s 0.0 0.02 0.05 \\
        --alpha_a 0.0 0.3 \\
        --t_start 5 10 \\
        --n_rollouts 30 \\
        --output_csv results/can/bc_rnn_denoiser/bc_rnn_a0_results.csv
"""
...
_GRIPPER_ACTION_DIM = 6   # index of gripper command in 7-dim Can action
_CAN_Z_THRESH      = 0.85
...
def _build_anchor_dict(obs, action, object_pose_t0, gripper_hist_buf, gripper_k, device):
    ...
    _phase  = 2 if (_closed and _obj_z > _CAN_Z_THRESH) else (1 if _closed else 0)
...
def parse_args():
    ...
    p.add_argument("--output_csv", type=str,
                   default="results/can/bc_rnn_denoiser/results.csv")
