"""Custom diffusion policy components for Robomimic Can.

This module is intentionally self-contained so the Can diffusion-policy
...
class CanDiffusionPolicy(nn.Module):
    """Noise-prediction network for the Can diffusion policy.
...
class CanTrajectoryDataset(Dataset):
    """Sliding-window Can dataset for a diffusion policy."""
...
def load_can_checkpoint(checkpoint_path: str, device: torch.device):
    """Load a checkpoint saved by train_mlp_diffusion_can.py."""
    ...
    if missing_keys:
        raise ValueError(
            "Checkpoint is not a custom Can diffusion-policy checkpoint. "
            f"Missing keys: {missing_keys}. "
            "Use a checkpoint produced by training/can/train_mlp_diffusion_can.py."
        )
