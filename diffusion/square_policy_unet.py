"""DiffusionPolicy-C: 1D Temporal UNet backbone for Robomimic Square.

Implements the convolutional (CNN) backbone from Chi et al. 2023.
Architecture is task-agnostic; only the checkpoint loader references Square.
"""
...
class UNetDiffusionPolicy(nn.Module):
    """DiffusionPolicy-C: 1D temporal UNet for Square task.
...
def load_unet_checkpoint(checkpoint_path: str, device: torch.device):
    from diffusion.square_policy import make_beta_schedule
