"""
Train the joint (state, action) UNet denoiser.

Uses JointUNet1D (FiLM-conditioned 1D temporal U-Net) — the same architecture
as the existing Lift joint checkpoints (joint_a0_lift.pt, joint_a7_lift.pt).
Saves a checkpoint with no "arch" key so the eval loader uses the UNet branch.
...
Examples:
    ...
    python -m diffusion.train_joint_denoiser_unet \\
        ...
        --anchor A0 --horizon 8 \\
        --output_path diffusion_models/joint_unet_can_a0.pt

    # Can — A7 anchor
    python -m diffusion.train_joint_denoiser_unet \\
        --bc_rnn_ckpt checkpoints/bc_rnn_can/bc_rnn_can/20260405211805/models/model_epoch_600.pth \\
        --anchor A7 --horizon 8 \\
        --output_path diffusion_models/joint_unet_can_a7.pt
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train joint UNet denoiser (JointUNet1D)")
    ...
    p.add_argument("--hdf5_path", type=str,
                   ...
                   help="Path to robomimic HDF5 demo file. Default: Can")
    ...
    p.add_argument("--output_path", type=str, default=None,
                   help="Output .pt path. Default: diffusion_models/joint_unet_<anchor>.pt")
...
def main():
    ...
    if args.output_path is None:
        ...
        args.output_path = f"diffusion_models/joint_unet_{args.anchor.lower()}.pt"
    ...
    # Model + Anchor
    from diffusion.joint_unet import JointUNet1D, joint_diffusion_loss
    ...
    lam_display = args.lam if args.lam is not None else "auto (Da/Ds)"
    print(f"\nTraining")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in loader:
            x0_state_clean  = batch["state"].to(device)
            x0_action_clean = batch["action"].to(device)
            x0_state_noisy  = batch["state_noisy"].to(device)
            x0_action_noisy = batch["action_noisy"].to(device)
    ...
    # Save checkpoint — no "arch" key → eval loader uses JointUNet1D branch
    ...
    torch.save(
        {
            ...
            # Architecture (no "arch" key → eval uses JointUNet1D)
            "state_dim":         ds.state_dim,
            "action_dim":        ds.action_dim,
            "horizon":           args.horizon,
