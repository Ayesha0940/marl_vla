"""
Train the action-only denoiser for Robomimic Square (NutAssemblySquare).
...
Examples:
    python -m diffusion.train_action_denoiser_square \\
        ...
        --anchor A7 \\
        --output_path diffusion_models/action_a7_square.pt

    # A0 anchor (no anchor)
    python -m diffusion.train_action_denoiser_square \\
        --bc_rnn_ckpt checkpoints/bc_rnn_square/seed3/bc_rnn_square_v3_seed3/20260418050524/models/model_epoch_800_NutAssemblySquare_success_0.8.pth \\
        --anchor A0 \\
        --output_path diffusion_models/action_a0_square.pt
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train action-only denoiser for Square")
    ...
    p.add_argument("--hdf5_path", type=str,
                   default="datasets/square/ph/low_dim_v141.hdf5",
                   help="Path to robomimic Square HDF5 demo file")
    p.add_argument("--anchor", type=str, default="A1",
                   choices=["A0","A1","A2","A3","A4","A5","A6","A7","A8"],
                   help="Anchor variant. Default: A1")
    p.add_argument("--horizon", type=int, default=8,
                   help="Temporal window length H. Default: 8 (matches Square action_horizon)")
    ...
    p.add_argument("--output_path", type=str, default=None,
                   help="Output .pt path. Default: diffusion_models/action_<anchor>_square.pt")
...
def main():
    ...
    if args.output_path is None:
        ...
        args.output_path = f"diffusion_models/action_{args.anchor.lower()}{suffix}_square.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in loader:
            ...
            if args.no_state_ctx:
                ...
                state_ctx = batch["state_noisy"].to(device)
