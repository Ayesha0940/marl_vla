#!/usr/bin/env bash
# BC RNN baseline evaluation with action and joint denoisers — Lift task, anchor A0
#
# Steps:
...
#
# Run from repo root:
#   bash run_bc_rnn_denoiser_lift_a0.sh
#
# Optional overrides via env:
#   EPOCHS=50    bash run_bc_rnn_denoiser_lift_a0.sh    # quick smoke test
#   N_ROLLOUTS=5 bash run_bc_rnn_denoiser_lift_a0.sh
#   DEVICE=cpu   bash run_bc_rnn_denoiser_lift_a0.sh
...
ACTION_CKPT="$REPO_ROOT/diffusion_models/action_a7_lift.pt"
JOINT_CKPT="$REPO_ROOT/diffusion_models/ablation/joint_lam01_a7.pt"
RESULTS_DIR="$REPO_ROOT/results/lift/bc_rnn_denoiser"
RESULTS_CSV="$RESULTS_DIR/bc_rnn_a7_results_lam1.csv"
...
ALPHA_S_LIST="${ALPHA_S_LIST:-0.0 0.02 0.04 0.05}"
ALPHA_A_LIST="${ALPHA_A_LIST:-0.0 0.1 0.2 0.3}"
...
cd "$REPO_ROOT"
python evaluation/eval_bc_rnn_denoiser_lift.py \
