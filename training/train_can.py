#!/usr/bin/env python
"""
Generic training script for can task that works across different machines and directories.
Usage: python train_can.py
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='Launch can training with optional seed and output directory overrides.'
    )
    ...
    parser.add_argument('--config', type=str, default=None,
                        help='Optional base config file to use instead of configs/bc_rnn_can.json')
...
def make_run_config(base_config_path, seed, output_dir):
    ...
    if 'experiment' in config and isinstance(config['experiment'], dict):
        base_name = config['experiment'].get('name', 'bc_rnn_can')
        suffix = os.path.basename(output_dir)
        config['experiment']['name'] = f"{base_name}_{suffix}"

    override_name = f"bc_rnn_can_{os.path.basename(output_dir)}.json"
...
def main():
    ...
    base_config_file = args.config or os.path.join(PROJECT_ROOT, 'configs/bc_rnn_can.json')
    if not os.path.exists(base_config_file):
        print("Generating config file...")
        gen_config_script = os.path.join(TRAINING_DIR, 'gen_config_can.py')
    ...
    if args.seed is not None or args.run_name is not None:
        ...
        output_dir = os.path.join(PROJECT_ROOT, 'checkpoints/bc_rnn_can', run_name)
