#!/usr/bin/env python3
"""
SA-MDP UNet diffusion policy trainer for Robomimic Square/NutAssemblySquare.

Identical to train_unet_diffusion_square.py except the loss adds an observation-
...
obs_tilde uses per-dim sigma normalization:
    sigma_max_norm = sigma_max / obs_std   # (obs_dim,)
    alpha          = Uniform(0, sigma_max_norm) per sample
    obs_tilde      = obs + alpha * randn_like(obs)

Usage:
    # Default run (kappa=1.0, sigma_max=0.05)
    python training/square/train_diffusion_samdp_square.py

    # kappa sweep
    python training/square/train_diffusion_samdp_square.py --kappa 0.3 --output_dir checkpoints/square_samdp_k03
    python training/square/train_diffusion_samdp_square.py --kappa 1.0 --output_dir checkpoints/square_samdp_k10
    python training/square/train_diffusion_samdp_square.py --kappa 3.0 --output_dir checkpoints/square_samdp_k30
"""
...
def default_config() -> dict:
    return {
        ...
        "task": "square",
        "train": {
            "data":        os.path.join(PROJECT_ROOT, "datasets/square/ph/low_dim_v141.hdf5"),
            "output_dir":  os.path.join(PROJECT_ROOT, "checkpoints/square_samdp_unet"),
            "num_epochs":  4500,
            "batch_size":  256,
            "num_workers": 2,
            "log_freq":    10,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SA-MDP UNet diffusion policy trainer — Square")
    ...
    p.add_argument(
        ...
        help="Max obs perturbation in raw state space (default 0.05). "
             "Converted per-dim to normalized space via sigma_max / obs_std.",
    )
    return p.parse_args()
...
def train(config: dict, seed: int = 0, device_arg: str = "auto") -> None:
    ...
    obs_std_tensor = torch.from_numpy(dataset.obs_std).float().to(device)
    sigma_max_norm = sigma_max / obs_std_tensor
