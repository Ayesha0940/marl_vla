#!/usr/bin/env python3
"""Train a UNet diffusion policy for Robomimic Square (x0-prediction).
...
Key design choices:
  ...
  - action_horizon=8 (matches robomimic square diffusion config; exec_horizon=2 at eval)

Usage:
    python training/square/train_unet_diffusion_square.py
    python training/square/train_unet_diffusion_square.py --num_epochs 6000 --lr 1e-4
"""
...
from diffusion.square_policy import (
    SquareTrajectoryDataset,
    make_beta_schedule,
    q_sample,
)
from diffusion.square_policy_unet import UNetDiffusionPolicy, save_unet_checkpoint
...
def default_config() -> dict:
    return {
        ...
        "train": {
            ...
            "output_dir": os.path.join(PROJECT_ROOT, "checkpoints/square_diffusion_policy_unet"),
            "num_epochs": 4500,
            "batch_size": 256,
            "num_workers": 2,
            "log_freq": 10,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train UNet diffusion policy for Square")
...
def train(config: dict, seed: int = 0, device_arg: str = "auto") -> None:
    ...
    dataset = SquareTrajectoryDataset(
