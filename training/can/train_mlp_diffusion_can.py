#!/usr/bin/env python3
"""Train a custom diffusion policy for Robomimic Can."""
...
from diffusion.can_policy import CanDiffusionPolicy, CanTrajectoryDataset, make_beta_schedule, q_sample
...
def default_config() -> dict:
    return {
        "algo_name": "diffusion",
        "task": "can",
        "experiment": {"name": "can_diffusion_policy"},
        "train": {
            "data": os.path.join(PROJECT_ROOT, "datasets/can/ph/low_dim_v141.hdf5"),
            "output_dir": os.path.join(PROJECT_ROOT, "checkpoints/can_diffusion_policy"),
            "num_epochs": 2000,
            "batch_size": 256,
            "num_workers": 2,
            "log_freq": 10,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a custom diffusion policy on Can")
...
def train(config: dict, seed: int = 0, device_arg: str = "auto", resume: str | None = None) -> None:
    ...
    if resume:
        ...
        # Load the true best loss from best_model.pt (stores averaged loss),
        # not from the epoch checkpoint (which stored the un-averaged sum before this fix).
        ...
        if os.path.isfile(best_model_path):
            ...
            best_loss = float(best_ckpt.get("loss", float("inf")))
        print(f"Resumed from {resume} (epoch {start_epoch - 1}, best_loss={best_loss:.6f})")
    ...
    end_epoch = start_epoch + int(train_cfg["num_epochs"]) - 1
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        if epoch == start_epoch or epoch % int(train_cfg["log_freq"]) == 0:
            print(f"Epoch {epoch:4d}/{end_epoch} | loss={epoch_loss:.6f} | best={best_loss:.6f}")
...
def main() -> None:
    ...
    if args.config is None:
        config_path = os.path.join(PROJECT_ROOT, "configs", "diffusion_can.json")
        save_config(config_path, config)
        print(f"Saved config to {config_path}")

    train(config, seed=args.seed, device_arg=args.device, resume=args.resume)
