#!/usr/bin/env bash
# BC RNN baseline evaluation with action and joint denoisers — Can task, anchor A7
#
# Steps:
...
#
# Run from repo root:
#   bash run_bc_rnn_denoiser_can_a7.sh
#
# Optional overrides via env:
#   EPOCHS=50    bash run_bc_rnn_denoiser_can_a7.sh    # quick smoke test
#   N_ROLLOUTS=5 bash run_bc_rnn_denoiser_can_a7.sh
#   DEVICE=cpu   bash run_bc_rnn_denoiser_can_a7.sh
...
ACTION_CKPT="$REPO_ROOT/diffusion_models/action_a7_can.pt"
JOINT_CKPT="$REPO_ROOT/diffusion_models/joint_a7_can.pt"
RESULTS_DIR="$REPO_ROOT/results/can/bc_rnn_denoiser"
RESULTS_CSV="$RESULTS_DIR/bc_rnn_a7_results.csv"
