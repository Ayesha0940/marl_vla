#!/usr/bin/env bash
# Action-only denoiser pipeline — Can task, A0 then A7
#
# Trains and evaluates ActionDenoisingUNet1D for both anchors sequentially.
#
# Run from repo root:
#   bash run_action_denoiser_can_all.sh
#
# Optional overrides via env:
#   EPOCHS=50 bash run_action_denoiser_can_all.sh    # quick smoke test
#   DEVICE=cpu bash run_action_denoiser_can_all.sh

set -euo pipefail

# ── Shared paths ─────────────────────────────────────────────────────────────
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BC_RNN_CKPT="$REPO_ROOT/checkpoints/bc_rnn_can/bc_rnn_can/20260405211805/models/model_epoch_600.pth"
